...
import { getResource } from "./resource/requestTranslate";

// eslint-disable-next-line @typescript-eslint/naming-convention
const { Position, window } = vscode;
const decorationType = window.createTextEditorDecorationType({after: {margin: '0 0 0 0.5rem'}});
let decorationsDebounce: NodeJS.Timeout, originDecorations = {},decorations = {}, regexpList = [];
  ...
  getResource().then((languageInfo) => {
    if (window.activeTextEditor) {
      ...
      lineText.forEach((text, index) => {
        lineTextCheck(text, index + 1, fileName, languageInfo);
      });
...
function decorate(text, packageInfo, color = '#67C23A') {
	...
	decorations[fileName][line].push({
		...
		range: new vscode.Range(new Position(line - 1, packageInfo.lineMatchIndex), new Position(line - 1, packageInfo.lineMatchIndex))
	});
}
	...
	Object.keys(fileOriginDecorations).forEach((line) => {
		if (!fileDecorations[line]) {
			fileOriginDecorations[line] = [{
				...
				range: new vscode.Range(new Position(Number(line) - 1, 1000), new Position(Number(line) - 1, 1000))
			}];
		}
	});
